...
| `OnPlaySpell`             | ❌           |                                                                                                                                                                      |
| `StartMainPhase`          | ❌           | Start of your main phase                                                                                                                                             |
| `StartOpponentMainPhase`  | ❌           | Start of your opponent's main phase                                                                                                                                  |
...
| `cost (cost1), (cost2), ...;` | [Ability Costs][Ability Cost]                                                                                                   |                                                                                                                                                      |
...
| `condition <condition>;`      | Condition that must be met in order to perform the effect. If not met, the effect is skipped/can't be performed.<br>            | `<condition>` is a [value function][Value Function] that is true if the returned number is greater than 0.                                           |
| `filter <filter>`             | [Filter Function][Filter Function]<br>Used by specific effect triggers to filter which cards can cause this ability to trigger. | Only supported by specific triggers (see [list of triggers](#triggers))                                                                              |
...
- `<effect name>` is the [type of effect][Effect List] to perform
...
- `<filter>` is a [filter function][Filter Function] that controls which cards can be targeted, when applicable
...
| `TargetCards`                | Target card(s) on either/both player's fields                                                                                                    |
| `AllPlayers`                 |                                                                                                                                                  |
| `AllLeaders`                 |                                                                                                                                                  |
| `TriggerCard`                | Card that triggered this effect<br>Used by the `OnOther` family of ability triggers                                                              |

[Ability Cost]: ./ability-costs.html
[Effect List]: ./effect-list.html
