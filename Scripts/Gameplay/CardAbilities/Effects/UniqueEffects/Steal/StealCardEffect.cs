...
{
    public class StealCardEffect : SveEffect
    {
        [EnumField("Target", width = 200), Order(1)]
        public SVEProperties.SVEEffectTarget target = SVEProperties.SVEEffectTarget.TargetOpponentCard;
        ...
        public override void Resolve(PlayerController player, int triggeringCardInstanceId, string triggeringCardZone, int sourceCardInstanceId, string sourceCardZone, Action onComplete = null)
        {
            ...
            {
                Debug.Log($"Effect {nameof(StealCardEffect)} is not currently supported");
                onComplete?.Invoke();
            });
        }
    }
